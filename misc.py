...
def cal_fmeasure(precision, recall):
    ...
    mean_fmeasure = np.mean([(1 + beta_square) * p * r / (beta_square * p + r) for p, r in zip(precision, recall)])

    return max_fmeasure,mean_fmeasure

def cal_fmeasure1(precision, recall):
    beta_square = 0.3
    fmeasure=[]
    for i in range(len(precision)):
        tmp = []
        for j in range(len(precision[i])):
            tmp1 = (1 + beta_square) * precision[i][j] * recall[i][j] / (beta_square * precision[i][j] + recall[i][j])
            tmp.append(tmp1)
        fmeasure.append(tmp)
    fmeasure = np.mean(fmeasure,0)
