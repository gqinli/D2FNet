...
def getImage(root='demo'):
    # make value in 0-10 because should in 'meter'
    D = cv2.imread(root, cv2.COLOR_BGR2GRAY) / 100
    # D = scio.loadmat(root)['smoothedDepth']/1000
...
if __name__ == "__main__":
    depth_Data = os.listdir('../data/STERE/depth/')
    for i in depth_Data:
        root = os.path.join('../data/STERE/depth', i)
        D, RD = getImage(root)
        ...
        cv2.imwrite(os.path.join('../data/STERE/hha1', i.split('.')[0]+'.jpg'), hha_complete)
        print(i,' is finshed')
