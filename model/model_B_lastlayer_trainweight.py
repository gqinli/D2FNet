...
class R3Net(nn.Module):
    def __init__(self, num_class=1):
        ...
        self.weight5 = weight_learn(1024, 128, 5)
    ...
    def forward(self, x, depth):
        ...
        w_ = self.weight5(conv7_depth)
        w7 = w_[:, 0].unsqueeze(1).unsqueeze(1).unsqueeze(1)
        w5 = w_[:, 1].unsqueeze(1).unsqueeze(1).unsqueeze(1)
        w4 = w_[:, 2].unsqueeze(1).unsqueeze(1).unsqueeze(1)
        w3 = w_[:, 3].unsqueeze(1).unsqueeze(1).unsqueeze(1)
        w2 = w_[:, 4].unsqueeze(1).unsqueeze(1).unsqueeze(1)
        ...
        ## fuse later layer
        # 1/32, w4
        R5 = torch.cat((f7 * w7, d7 * w7), dim=1)
        R5 = self.R5(R5)  # 256, 1/32

        # 1/16
        R4 = torch.cat((up7[0] * w7, f5 * w5), dim=1)  # 256, 1/16
        R4 = self.R4(R4)

        # 1/8
        R3 = torch.cat((up7[1] * w7, up5[0] * w5, f4 * w4), dim=1)
        R3 = self.R3(R3)  # 256, 1/8

        # 1/4
        R2 = torch.cat((up7[2] * w7, up5[1] * w5, up4[0] * w4, f3 * w3), dim=1)
        R2 = self.R2(R2)  # 128, 1/4

        # 1/4
        R1 = torch.cat((up7[3] * w7, up5[2] * w5, up4[1] * w4, up3[0] * w3, f2 * w2), dim=1)
